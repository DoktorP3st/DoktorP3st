<div align="center">

# Pestovich

[![fr](https://img.shields.io/badge/lang-fr-blue.svg)](https://github.com/Lekarov/Lekarov/blob/main/README.fr.md)
[![en](https://img.shields.io/badge/lang-en-red.svg)](https://github.com/Lekarov/Lekarov/blob/main/README.md)

**Développeur Python · Outils self-hosted · Extensions navigateur · Macros jeux · Streaming Twitch**

Outils locaux & open source — sans serveur, sans abonnement.

![Python](https://img.shields.io/badge/Python-3.10+-3776AB?style=flat-square&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-ES6+-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat-square&logo=fastapi&logoColor=white)
![Chrome](https://img.shields.io/badge/Chrome-Extension-4285F4?style=flat-square&logo=googlechrome&logoColor=white)
![Self-hosted](https://img.shields.io/badge/self--hosted-toujours-success?style=flat-square)

</div>

---

## Extensions navigateur

### [ZeroTrace by Pestovich](https://github.com/Lekarov/ZeroTrace-By-Pestovich) — Extension Chrome / Brave

> Efface toutes les traces du navigateur en 1 clic — cookies, cache, historique, storage — avec whitelist pour protéger les sessions importantes.

![Version](https://img.shields.io/badge/version-1.6.1-e63946?style=flat-square)
![Manifest](https://img.shields.io/badge/Manifest-V3-00c896?style=flat-square)
![Licence](https://img.shields.io/badge/licence-GPL--v3-6b7491?style=flat-square)
![Chrome](https://img.shields.io/badge/Chrome-Compatible-4285F4?style=flat-square&logo=googlechrome&logoColor=white) ![Brave](https://img.shields.io/badge/Brave-Compatible-FB542B?style=flat-square&logo=brave&logoColor=white)
![Langues](https://img.shields.io/badge/langues-6-orange?style=flat-square)

Nettoyage 1 clic · Whitelist de sites (cookies + localStorage/indexedDB) · Gestionnaire de cookies · Auto-clean au démarrage · Backup & Restore

---

## Outils self-hosted & automation

### [Purgearr](https://github.com/Lekarov/Purgearr) — Gestionnaire de médias self-hosted

> Libère de l'espace. Garde l'essentiel. Une interface pour toute ta bibliothèque Jellyfin + Radarr + Sonarr + Transmission.

![Status](https://img.shields.io/badge/status-bêta-orange?style=flat-square)
![Python](https://img.shields.io/badge/Python-3.10+-3776AB?style=flat-square&logo=python&logoColor=white)
![Licence](https://img.shields.io/badge/licence-MIT-6b7491?style=flat-square)
![Jellyfin](https://img.shields.io/badge/Jellyfin-00A4DC?style=flat-square&logo=jellyfin&logoColor=white) ![Radarr](https://img.shields.io/badge/Radarr-FFC230?style=flat-square) ![Sonarr](https://img.shields.io/badge/Sonarr-35C5F4?style=flat-square) ![Transmission](https://img.shields.io/badge/Transmission-CC0000?style=flat-square)
![Plateforme](https://img.shields.io/badge/Raspberry%20Pi-NAS-c51a4a?style=flat-square&logo=raspberrypi&logoColor=white)

Détection contenus jamais vus · Scanner torrents morts · Whitelist · Détection hardlinks · Multi-utilisateurs

### [WaveRouter](https://github.com/Lekarov/WaveRouter) — Routeur audio de jeu

> Routage automatique de l'audio des jeux vers Elgato Wave Link sur Windows — plus besoin de changer le périphérique manuellement.

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Windows](https://img.shields.io/badge/Windows-0078D6?style=flat-square&logo=windows&logoColor=white)
![Elgato](https://img.shields.io/badge/Elgato-Wave%20Link-DA291C?style=flat-square)

### [MarkerBridge](https://github.com/Lekarov/MarkerBridge) — Pont OBS → Premiere

> Convertit les chapitres OBS en marqueurs Adobe Premiere Pro (XML FCP7) — GUI + CLI, Windows.

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![OBS](https://img.shields.io/badge/OBS-Chapitres-302E31?style=flat-square&logo=obsstudio&logoColor=white)
![Premiere](https://img.shields.io/badge/Adobe-Premiere%20Pro-9999FF?style=flat-square&logo=adobepremierepro&logoColor=white)

### [G2G-2FA-Generator](https://github.com/Lekarov/G2G-2FA-Generator) — Générateur de codes TOTP

> Génère des codes 2FA (TOTP) en direct à partir des exports CSV de comptes G2G.

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)

---

## Twitch / Streaming — Overlays & outils

### [Pestovich Overlay Alerts](https://github.com/Lekarov/Pestovich-Overlay-Alerts-V1) — Widget d'alertes stream

> Blood Alerts, édition Diablo — widget d'alertes pour Twitch/OBS/StreamElements.

![Plateforme](https://img.shields.io/badge/StreamElements-OBS-9146FF?style=flat-square&logo=twitch&logoColor=white)
![Langage](https://img.shields.io/badge/HTML%2FCSS%2FJS-black?style=flat-square)

### [NOVA](https://github.com/Lekarov/Overlay-Twitch-NOVA) — Overlay Chat Twitch

> Overlay de chat constellation pour StreamElements — chaque message devient une étoile qui rejoint un fil lumineux continu. Fond ciel nocturne optionnel, ou mode cadre seul pour superposer directement sur n'importe quel contenu.

![Plateforme](https://img.shields.io/badge/StreamElements-OBS-9146FF?style=flat-square&logo=twitch&logoColor=white)
![Langage](https://img.shields.io/badge/HTML%2FCSS%2FJS-black?style=flat-square)

### [CIPHER](https://github.com/Lekarov/Overlay-Twitch-CIPHER) — Overlay Chat Twitch

> Overlay de chat cyberpunk pour StreamElements — scanlines, glow néon, barres de signal, coins en L dans la couleur de chaque viewer.

![Plateforme](https://img.shields.io/badge/StreamElements-OBS-9146FF?style=flat-square&logo=twitch&logoColor=white)
![Langage](https://img.shields.io/badge/HTML%2FCSS%2FJS-black?style=flat-square)

### [StreamOracle](https://github.com/Lekarov/StreamOracle) — Assistant IA vocal

> Assistant IA vocal pour streamers Twitch — wake word, transcription Whisper locale (GPU), réponses Claude, synthèse vocale Microsoft Neural.

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Whisper](https://img.shields.io/badge/faster--whisper-GPU-74aa9c?style=flat-square)
![Claude](https://img.shields.io/badge/Claude-Anthropic-D97706?style=flat-square)

### [TAC Players Music Overlay](https://github.com/Lekarov/TAC-Players-Music-Overlay) — Now Playing OBS

> Lecteur audio Python avec overlay OBS live — pochette, titre, barre de progression animée.

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![OBS](https://img.shields.io/badge/OBS-Browser%20Source-302E31?style=flat-square&logo=obsstudio&logoColor=white)

### [BubuRog Die Counter](https://github.com/Lekarov/BubuRog-Die-Counter) — Compteur de morts Twitch

> Overlay de comptage de morts en temps réel pour BubuRog — incrémentation clavier, animations, StreamElements ready.

![Plateforme](https://img.shields.io/badge/StreamElements-OBS-9146FF?style=flat-square&logo=twitch&logoColor=white)
![Langage](https://img.shields.io/badge/CSS%2FJS-black?style=flat-square)

### [Mort-o-Mètre](https://github.com/Lekarov/Mort-o-M-tre) — Compteur de morts interactif

> Overlay Twitch interactif pour tracker les morts, fails ou événements en temps réel — StreamElements & OBS ready.

![Plateforme](https://img.shields.io/badge/StreamElements-OBS-9146FF?style=flat-square&logo=twitch&logoColor=white)
![Langage](https://img.shields.io/badge/HTML%2FCSS%2FJS-black?style=flat-square)

### Overlays caméra & chat gratuits

Overlays StreamElements gratuits — chat, cadre caméra (16:9 et 1:1), OBS ready, sans attribution requise.

[![Chat Overlay](https://img.shields.io/badge/Chat%20Overlay-v1-9146FF?style=flat-square&logo=twitch&logoColor=white)](https://github.com/Lekarov/Pestovich-Overlay-Chat-V1)
[![Cam Overlay](https://img.shields.io/badge/Cam%20Overlay-v1-9146FF?style=flat-square&logo=twitch&logoColor=white)](https://github.com/Lekarov/Pestovich-Overlay-Cam-V1)

---

## Création de contenu

### [TAC MP4 Studio](https://github.com/Lekarov/TAC-MP4-Studio) — Générateur de clips musicaux réactifs

> Génère des vidéos musicales réactives frame by frame, beat by beat — export MP4 HD/vertical, sans cloud.

![Version](https://img.shields.io/badge/version-1.9.1-7c3aed?style=flat-square)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![FFmpeg](https://img.shields.io/badge/FFmpeg-NVENC%20%2F%20libx264-007808?style=flat-square)

---

## Automation de jeu

### [DiabloIV Macro](https://github.com/Lekarov/DiabloIV-Macro-ByPestovich) — Overlay d'automation

> Overlay d'automation pour Barbare Tourbillon Diablo IV — cooldowns, maintien WW, potion automatique. 5 langues.

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Langues](https://img.shields.io/badge/langues-5-orange?style=flat-square)

---

## Stack technique

| Domaine | Outils |
|---|---|
| **Backend** | `FastAPI` `Uvicorn` `SQLAlchemy` `APScheduler` |
| **IA / Vocal** | `anthropic` `faster-whisper` `edge-tts` `sounddevice` |
| **Audio / Vidéo** | `librosa` `opencv-python` `Pillow` `scipy` `FFmpeg` |
| **Interface** | `customtkinter` `tkinter` `Jinja2` |
| **Automation** | `pynput` `ctypes` `threading` |
| **Extensions** | `Manifest V3` `Chrome Extensions API` `chrome.browsingData` |
| **Web / Stream** | `HTML5` `CSS3` `JavaScript` `StreamElements` `OBS` |
| **Plateforme** | `Windows 10 / 11` `Raspberry Pi` `Python 3.10+` |

---

<div align="center">

*Tous les projets sont locaux, open source et sans abonnement.*

</div>
